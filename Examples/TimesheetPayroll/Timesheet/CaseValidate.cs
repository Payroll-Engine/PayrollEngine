...
public static class TimesheetCase
{
        ...
        where TTimesheet : Timesheet, new()
    {
        ...
        if (!function.AdminUser)
        {
            ...
            {
                function.AddCaseIssue($"Timesheet change date {start.Value:d} is before calendar start {period.Start:d}");
                return true;
            }
        }
        ...
        if (workTime <= 0)
        {
            function.AddCaseIssue("Missing timesheet regular duration.");
            return true;
        }

        if (workday.MaxWorkTime < workday.MinWorkTime)
        {
            function.AddCaseIssue("Invalid working time maximum.");
            return true;
        }
        ...
        {
            function.AddCaseIssue("Break time maximum must be less than the regular working time.");
            return true;
        }

        if (workday.BreakMax < workday.BreakMin)
        {
            function.AddCaseIssue("Invalid break time maximum.");
            return true;
        }
        ...
        {
            function.AddCaseIssue("Invalid working time step size.");
            return true;
        }

        if (60m % workday.WorkTimeStep != 0)
        {
            function.AddCaseIssue("Working time step must be a part of 60 minutes.");
            return true;
        }
...
public static partial class TimesheetWorkTimeCase
{
        ...
        where TWorkTime : WorkTime, new()
    {
        ...
        if (duplicate)
        {
            function.AddCaseIssue($"Workday {workTime.WorkTimeDate:d} already submitted.");
            return true;
        }
        ...
        if (lookupDay == false)
        {
            function.AddCaseIssue($"{workTime.WorkTimeDate:d} is not a working day.");
            return true;
        }
        // day off by calendar weekday
        if (!function.IsCalendarWorkDay(workTime.WorkTimeDate) && lookupDay != true)
        {
            function.AddCaseIssue($"{workTime:dddd} is not a working day.");
            return true;
        }
        ...
        if (!function.AdminUser)
        {
            ...
            {
                function.AddCaseIssue($"Work time date {workTime.WorkTimeDate:d} is before calendar start {period.Start:d}");
                return true;
            }
        }
        ...
        {
            function.AddCaseIssue($"Missing working time (start={timesheet.StartTime}, end={timesheet.EndTime}).");
            return true;
        }
        // work time min
        if (timesheet.MinWorkTime > 0 && workTime.WorkTimeHours < timesheet.MinWorkTime)
        {
            function.AddCaseIssue($"Working time {workTime.WorkTimeHours:0.##} is less than {timesheet.MinWorkTime:0.##} hours.");
            return true;
        }
        // work time max
        if (timesheet.MaxWorkTime > 0 && workTime.WorkTimeHours > timesheet.MaxWorkTime)
        {
            function.AddCaseIssue($"Working time {workTime.WorkTimeHours:0.##} is more than {timesheet.MaxWorkTime:0.##} hours.");
            return true;
        }
        ...
        {
            function.AddCaseIssue($"Break time {workTime.WorkTimeBreak:0.##} is less than {timesheet.BreakMin:0.##} minutes.");
            return true;
        }
        // break time max
        if (timesheet.BreakMax > 0 && workTime.WorkTimeBreak > timesheet.BreakMax)
        {
            function.AddCaseIssue($"Break time {workTime.WorkTimeBreak:0.##} is more than {timesheet.BreakMax:0.##} minutes.");
            return true;
        }
