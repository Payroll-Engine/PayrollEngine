...
- **Employment level scaling** — `BaseSalary = Salary × EmploymentLevel`; part-time factor applied via No-Code action expression
- **Overtime restricted to full-time** — `OvertimeHours` case hidden via `availableActions`; WT 120 guarded independently by `? ^^EmploymentLevel >= 1.0`
- **`CalendarPeriod` missing value** — returns `0` when no value is entered; no `HasValue` guard needed in the overtime calculation
- **`^# PartTime Payroll Example
...
runs. Note that `^$` names must be space-free — hence `BaseSalary` rather than `Base Salary`.
 ...
 wage type reference** — reads a value already computed in the same payrun period; name must be space-free
- **ClusterSets for retro isolation** — `Retro` cluster contains only the base wage types; delta WTs are `Legal` only to avoid double-counting in retro runs
- **`GetWageTypeRetroResultSum()`** — collects the accumulated diff between stored and recalculated retro values; callable directly from No-Code `valueActions`
- **Retroactive level change** — `created` date controls when a case change becomes visible; the engine schedules the retro run automatically
